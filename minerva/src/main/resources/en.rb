...
v.webkit=Page contains webkit CSS dirt.
v.colors=Page contains color: $0
...
exportInfo1=If you select "all books", the feature tree and internal books will not be included.
exportInfo2=If you select customer "-", no tags will be taken into account, i.e. <i>all</i> pages will be exported.
...
duplicatePage1=Complete copy
duplicatePage0=Copy without help keys, tags and cross-book links
duplicatePageHint=Button 1 is useful when splitting a page. Button 2 is useful if the page should serve as a template.
...
turnOffCustomerMode=Turn off customer mode!
customerModeIsOff=Customer mode is not active
...
qbofftext=Show all results
buchfilter=Filter
crossBookLinksMenu=Link pages
crossBookLinks=Link pages from other books
AddPage=Add page
NoLinkedPages=No pages have been linked yet.
NoPagesToAdd=There are no pages to add.
editContent=Edit content
BrokenLinks=Broken Links
brokenLinksInfo=The above-mentioned page contains links to the following pages, which are, however, inaccessible.
...
deepSearch2=Start the deep book search.
...
schnellButtonLeiste=Quick buttons bar
...
copyToWorkspaceShort=Copy to workspace
Transfer=Transfer
...
Ausblenden=Hide
AusblendenRueckgaengig=Unhide
ZeigeAlle=Show all
ZeigeAktive=Show active
AlleEinblenden=Unhide all
BLSiAbfr=Are you sure?
Mehrfachauswahl=Multi select
selectNone=Select none
action=Action
execute=Execute
selectThesePages=Select these ${bt} pages
